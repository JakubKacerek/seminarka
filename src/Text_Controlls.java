...
public class Text_Controlls {
    ...
    static void Empty(Points[] objects) {
        ...
        System.out.println("      - Zapsat body ručně - \"input\" (!) " +
                                        "\n                  POZOR! Zadávání ukončíš tak, že všechny zadané hodnoty budou nula!" +
                                        "\n                  Po ukončení zadávání, program automaticky doplní zbývající body automaticky!");
        ...
        switch (input) {
            case "input" -> {
                System.out.println("Prosím zadej body X, Y, Z a váhu bodu: ");
                Points_Generating.inputPoints(objects);
            }
            case "generate" -> {
    ...
    static void filled(Points[] objects) {
        ...
        System.out.println("      - Zobrazit všechny body - \"write\"");
        ...
        switch (input) {
            case "beg" -> {
                Points_Sorter_From_Beginning.quickSort(objects, 0, objects.length - 1);
                System.out.println("Chcete seřazené body vypsat? A/N");
                input = sc.nextLine();
                if (input.equals("A")) {
                    Points_Sorter_From_Beginning.printArray(objects, objects.length);
                    filled(objects);
                } else {
                    filled(objects);
                }
